...
function moveElement(elementID,final_x,final_y,interval) {
	...
	if (xpos < final_x) {
		var disc = Math.ceil((final_x - xpos)/10);
		xpos += disc;
	}
	if (xpos > final_x) {
		var disc = Math.ceil((xpos - final_x)/10);
		xpos -= disc;
	}
	if (ypos < final_y) {
		var disc = Math.ceil((final_y - ypos)/10);
		ypos += disc;
	}
	if (ypos > final_y) {
		var disc = Math.ceil((ypos - final_y)/10);
		ypos -= disc;
	}
